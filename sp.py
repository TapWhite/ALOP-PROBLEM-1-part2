...
def aper(image,xc,yc,apr,skyrad,badpix=[0,0],prnt=False, silent=False, flux=True, exact = False, nan = False, setskyval = [], readnoise = [], meanback = False, clipsig=3., maxiter=5.,converge_num=0.02, minsky = 20.):
...
INPUTS:
 ...
 EXAMPLE:
       ...
       Poisson counts ( which would mean gain = 1), and the sky value is already known
       to be 1.3, and that the range [-32767,80000] for bad low and bad high
       pixels + output to terminal
      

       [flux, sky]=aper( im, xc=[234.2], yc=[344.3], apr=[3,5], skyrad=[-1], badpix=[-32767,80000], exact=True, flux=True, setskyval = [1.3])
	...
	for i in numpy.arange(0,Nstars):
		...
		if skipstar == 0:
			...
			if Ng > 0:
                                ...
                                error2[g] = numpy.max(numpy.array([[apmag[g]],[numpy.zeros(Ng)]]))  #Random photon noise 
	...
	if silent == False:
            ...
            for i in numpy.arange(Naper):
                ...
                for j in numpy.arange(Nstars):
                    print(xc[j],"\t",yc[j],"\t",mags[j,i],"\t",sky[j])
